...
namespace AsadorMoron.ViewModels.Establecimientos
{
    public class ArticulosViewModel : ViewModelBase
    {
        ...
        {
            bool result = await App.customDialog.ShowDialogConfirmationAsync(AppResources.App, AppResources.PreguntaDesactivarProductos, AppResources.No, AppResources.Si);
            Desactivar2(result);
        }
        ...
        private void Desactivar2(bool hacer)
        {
            try
            {
                if (hacer)
                        ...
                        if (result != null)
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    do
                                    {
                                        if (string.IsNullOrEmpty(worksheet.Range["B" + indice].Text))
                                            seguir = false;
                                        else
                                        {
                                            int idCat = 0;
                                            Categoria cat = categorias.Find(pq => pq.nombre.Equals(worksheet.Range["B" + indice].Text.ToUpper()));
                                            if (cat == null)
                                            {
                                                ...
                                                c.idTipo = 1;
                                                c.nombre = worksheet.Range["B" + indice].Text.ToUpper();
                                                c.nombre_eng = worksheet.Range["B" + indice].Text.ToUpper();
                                                c.nombre_fr = worksheet.Range["B" + indice].Text.ToUpper();
                                                c.nombre_ger = worksheet.Range["B" + indice].Text.ToUpper();
                                            ...
                                            p.alergenos = "";
                                            p.Cantidad = 0;
                                            p.categoria = worksheet.Range["B" + indice].Text.ToUpper();
                                            if (string.IsNullOrEmpty(worksheet.Range["E" + indice].Text))
                                                p.descripcion = "";
                                            else
                                                p.descripcion = worksheet.Range["E" + indice].Text.ToUpper();
                                            p.estado = 1;
                                            p.estadoCategoria = 1;
                                            p.idCategoria = idCat;
                                            ...
                                            p.ingredientes = "";
                                            p.nombre = worksheet.Range["A" + indice].Text.ToUpper();
                                            p.opciones = "";
                                            p.precio = double.Parse(worksheet.Range["D" + indice].Value.ToString().Replace(".", ","));
                                            ...
                                            if (worksheet.Range["F" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 1;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["G" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 2;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["H" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 3;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["I" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 4;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["J" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 5;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["K" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 6;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["L" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 7;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["M" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 8;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["N" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 9;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["O" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 10;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["P" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 11;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["Q" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 12;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["R" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 13;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (worksheet.Range["S" + indice].Value.ToString().Equals("1"))
                                            {
                                                AlergenosModel o = new AlergenosModel();
                                                o.id = 14;
                                                p.listadoAlergenos.Add(o);
                                            }
                                            if (!string.IsNullOrEmpty(worksheet.Range["C" + indice].Text))
                                            {
                                                bool seguir2 = true;
                                                do
                                                {
                                                    OpcionesModel o = new OpcionesModel();
                                                    o.opcion = worksheet.Range["C" + indice].Text;
                                                    o.precio = worksheet.Range["D" + indice].Value.ToString().Replace(".", ",");
                                                    p.listadoOpciones.Add(o);
                                                    indice++;
                                                    if (string.IsNullOrEmpty(worksheet.Range["A" + indice].Text) && !string.IsNullOrEmpty(worksheet.Range["C" + indice].Text))
                                                        seguir2 = true;
                                                    else if (!string.IsNullOrEmpty(worksheet.Range["A" + indice].Text))
                                                        if (worksheet.Range["A" + indice].Text.ToUpper().Equals(p.nombre))
                                                            seguir2 = true;
                                                        else
                                                        {
                                                            seguir2 = false;
                                    ...
                                    await Filtrar();
                                    await ActualizaProductos();
                                    await App.customDialog.ShowDialogAsync(AppResources.ImportacionCartaOK.Replace("%1",categorias.Count.ToString()).Replace("%2",(indice - 2).ToString()), AppResources.App, AppResources.Cerrar);
                                }
        ...
        private async void EliminarProductoCommandExecute(object parametro)
        {
            ...
            if (result)
            {
                Comida r = (Comida)parametro;
                r.articulo.eliminado = 1;
        ...
        public ICommand cmdDesactivar { get { return new Command(Desactivar); } }
        ...
        public ICommand HeaderTapped { get { return new Command((parametro)=>headerTapped(parametro)); } }
        #endregion
