...
namespace AsadorMoron.ViewModels.Clientes
{
    public class PinViewModel : ViewModelBase
    {
        ...
        public ICommand CommandGenerarNuevoPIN { get { return new Command(GenerarNuevoPIN); } }

        private async void GenerarNuevoPIN(object obj)
        {
            try
            {
                await initGuardarPin();
            }
            catch (Exception ex)
            {
                // 
        ...
        private async void Guardar()
        {
            try
            {
                ...
                {
                    try { App.userdialog.ShowLoading(AppResources.ComprobandoPIN, MaskType.Black); } catch (Exception) { }
                    await Task.Delay(200);
                    //MainThread.BeginInvokeOnMainThread(() => userdialogLogin.ShowLoading("Cargando....", MaskType.Black));
                    await Task.Run(async () => { await initComprobacion(); }).ContinueWith(res => MainThread.BeginInvokeOnMainThread(() =>
                    {
                        if (ok)
                        {
                            //if (string.IsNullOrEmpty(App.DAUtil.UsuarioNoVerificado.nombre))
                            //    App.DAUtil.Usuario = App.DAUtil.UsuarioNoVerificado;
                            if (App.DAUtil.UsuarioNoVerificado != null)
                            {
                                App.ResponseWS.verificado(App.DAUtil.UsuarioNoVerificado.email);
                            }
                            else
                                App.ResponseWS.verificado(Preferences.Get("email", ""));
                        ...
                        else
                        {
                            ...
                            App.customDialog.ShowDialogAsync(App.MensajesGlobal.Where(p => p.clave.Equals("pin_incorrecto")).FirstOrDefault<MensajesModel>().valor, AppResources.Informacion, AppResources.Cerrar);
                            PIN = string.Empty;
                ...
                {
                    await App.customDialog.ShowDialogAsync(AppResources.IntroduzcaPin, AppResources.App, AppResources.Cerrar);
                }
            }
            catch (Exception ex)
            {
        ...
        private void IrALogin()
        {
            App.userdialog.ShowLoading(AppResources.Cargando);
                MainThread.BeginInvokeOnMainThread(async() =>
                {
                    ...
                    await App.DAUtil.NavigationService.LogOutAppAsync<LoginViewModel>(null);
                });
        }

        private async Task initComprobacion()
        {
            try
            {
                ok = await App.ResponseWS.verificaPIN(Preferences.Get("email", ""), PIN);
                if (ok)
                    Preferences.Set("PIN", "");
            }
            catch (Exception ex)
        ...
        private async Task initGuardarPin()
        {
            try
            {
                ...
                String error = App.ResponseWS.actualizaPIN(nuevoPIN, Preferences.Get("email", ""));
                if (!error.Contains("ERROR"))
                {
                    EnviaEmail(nuevoPIN);
                    await App.customDialog.ShowDialogAsync(AppResources.PinEnviado, AppResources.App, AppResources.Cerrar);
        ...
        private void EnviaEmail(string nuevoPIN)
        {
            try
            {
                ...
                App.DAUtil.EnviaEmail(Preferences.Get("email", ""), "Verifica tu email", cuerpo);
            }
            catch (Exception ex)
            {
                // 
