    ...
    public class RegistroViewModel : ViewModelBase
    {
            ...
            try
            {
                ...
                try
                {
                    if (!HaEntrado)
                    {
                        ...
                        //Zonas = new ObservableCollection<ZonaModel>(App.DAUtil.getZonas());
                        Nombre = "";
                        Apellidos = "";
                        Imagen = "profile.png";
        ...
        private async Task Guardar()
        {
            try
            {
                ...
                //{
                //    if (ok)
                //        App.DAUtil.NavigationService.NavigateToAsync<PinSMSViewModel>();
                //}));
            }
            catch (Exception ex)
            {
        ...
        private async Task initGuardar()
        {
            try
            {
                ...
                {
                    App.userdialog.HideLoading();
                    await App.customDialog.ShowDialogAsync(AppResources.NombreApObligatorio, AppResources.App, AppResources.Cerrar);
                }
                else if (FechaNacimiento == null)
                {
                    App.userdialog.HideLoading();
                    await App.customDialog.ShowDialogAsync(AppResources.FNObligatorio, AppResources.App, AppResources.Cerrar);
                }
                ...
                else if (Pass.Equals(RePass))
                {
                    ...
                    else
                    {
                        ...
                        if (continua)
                        {
                            ...
                            usu.email = Email;
                            usu.fechaAlta = DateTime.Today;
                            usu.fechaNacimiento = FechaNacimiento;
                            if (subirFoto)
                                usu.foto = $"{ResponseServiceWS.urlPro}images/clientes/" + g + ".jpg";
                            else
                                usu.foto = $"{ResponseServiceWS.urlPro}images/clientes/perfil.png";

                            usu.nombre = Nombre;
                            usu.nombreCompleto = Nombre + " " + Apellidos;
                            usu.password = Pass;
                            ...
                            //Xamarin.Essentials.Preferences.Set("PIN", PIN);
                            ...
                            if (!error.Contains("ERROR"))
                            {
                                ...
                                usu.idUsuario = int.Parse(error);
                                //Gestión del PIN
                                //App.DAUtil.UsuarioNoVerificado = usu;
                                App.DAUtil.Usuario = usu;
                                if (TieneCodigoAmigo && !string.IsNullOrEmpty(Codigo))
                                {
                                    GuardaPromoAmigo amigo2 = new GuardaPromoAmigo();
                                    amigo2.codigo = Codigo;
                            ...
                            {
                                if (error.Contains("verificado"))
                                {
                                    await App.customDialog.ShowDialogAsync("Su usuario ya está registrado, pero no está verificado, recibirá un email con un PIN de 4 cifras para validarlo" + Environment.NewLine + "Por favor, si no lo recibe, mire en su carpeta de Correo no deseado", AppResources.App, AppResources.Cerrar);
                        ...
                        {
                            App.userdialog.HideLoading();
                            await App.customDialog.ShowDialogAsync("El código de amigo introducido no es válido", AppResources.App, AppResources.Cerrar);
                        }
                    }
                }
                else
        ...
        private void EnviaEmail(string nuevoPIN)
        {
            try
            {
                ...
                cuerpo += "             <tr style=\"border-collapse:collapse;\"> ";
                cuerpo += "              <td align=\"center\" style=\"padding:0;Margin:0;\"><h1 style=\"Margin:0;line-height:36px;mso-line-height-rule:exactly;font-family:tahoma, verdana, segoe, sans-serif;font-size:30px;font-style:normal;font-weight:bold;color:#212121;\">¡Gracias por registrarse en Asador Morón!</h1></td> ";
                cuerpo += "             </tr> ";
                cuerpo += "             <tr style=\"border-collapse:collapse;\"> ";
        ...
        private bool EnviaEmail()
        {
            try
            {
                ...
                return App.DAUtil.EnviaEmail(Email, AppResources.VerificaEmail, cuerpo);
            }
            catch (Exception ex)
            {
                // 
        ...
        #region Comandos
        public AsyncRelayCommand  CommandGuardar { get { return new AsyncRelayCommand( async () => await Guardar()); } }
        ...
        {
            get { return textoPromo; }
            set
            {
                if (textoPromo != value)
                {
                    textoPromo = value;
                    OnPropertyChanged(nameof(TextoPromo));
                }
            }
        }
        ...
        PromocionAmigoModel promo = null;
