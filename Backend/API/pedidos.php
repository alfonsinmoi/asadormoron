...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['historicoAdmin'])) {
        ...
        $sql = $dbConn->prepare("SELECT DISTINCT
            ...
            INNER JOIN qo_users u ON (p.idUsuario=u.id)
            LEFT JOIN qo_repartidores re ON (re.id=p.idRepartidor)
            INNER JOIN (SELECT idPedido, sum(cantidad*precio) total FROM qo_pedidos_detalle WHERE pagadoConPuntos=0 GROUP BY idPedido) d ON (d.idPedido=p.id)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // =========================================================================
    // Incrementar contador de pollos asados si el pedido contiene "POLLO ASADO"
    // =========================================================================
    if ($postId && isset($input['lineasPedidos']) && is_array($input['lineasPedidos'])) {
        $totalPollos = 0;
        foreach ($input['lineasPedidos'] as $linea) {
            $nombreProducto = strtoupper($linea['nombreProducto'] ?? '');
            if (strpos($nombreProducto, 'POLLO ASADO') !== false) {
                $totalPollos += intval($linea['cantidad'] ?? 1);
            }
        }
        ...
        if ($totalPollos > 0) {
            ...
            // Incrementar contador de pollos
            $sqlPollo = $dbConn->prepare("SELECT id, cantidad FROM qo_contador_pollos WHERE idEstablecimiento = ? AND fecha = ? FOR UPDATE");
            $sqlPollo->execute([$idEst, $fecha]);
            $resultPollo = $sqlPollo->fetch(PDO::FETCH_ASSOC);

            if ($resultPollo) {
                $sqlUpdatePollo = $dbConn->prepare("UPDATE qo_contador_pollos SET cantidad = cantidad + ? WHERE id = ?");
                $sqlUpdatePollo->execute([$totalPollos, $resultPollo['id']]);
            } else {
                $sqlInsertPollo = $dbConn->prepare("INSERT INTO qo_contador_pollos (idEstablecimiento, fecha, cantidad) VALUES (?, ?, ?)");
                $sqlInsertPollo->execute([$idEst, $fecha, $totalPollos]);
            }
        }
    }
